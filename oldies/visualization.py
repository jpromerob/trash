...
class Oscilloscope:
    ...
    def get_counts(self,spike_count):
        ...
        while not self.spike_q.empty():
            ...
            print(f"{spike_count[0]}\t{spike_count[1]}\t\t{spike_count[2]}\t{spike_count[3]}")

            if int(spike_count[0])>th and int(spike_count[1])<th:
                self.line_plt_lr = self.line_plt_lr[1:]+"^"
            if int(spike_count[0])<th and int(spike_count[1])>th:
                self.line_plt_lr = self.line_plt_lr[1:]+"_"

            if int(spike_count[2])>th and int(spike_count[3])<th:
