...
def create_weight_list(w_fovea, w, h):
    ...
    for y in range(h):
        for x in range(w):
            ...
            x_weight = abs(-w_fovea*(x/(w/2)-1))
            y_weight = abs(-w_fovea*(y/(h/2)-1))
            print(f"{x+0.5}/{w}\t{y+0.5}/{h}")
            print(f"{w_fovea}\t{x_weight}\t{y_weight}")
            ...
            for post_idx in range(4):
                ...
                print("{:0.2f}\t{:0.2f}\t{:0.2f}\t{:0.2f}".format(x+0.5,y+0.5, post_idx, weight))
